...
public class EnergyCostsTests : TestBase
{
    ...
    public async Task ShouldCalculateEnergyCostForSameHour()
    {
        ...
        // Baseline measurement (also creates the PM sensor)
    ...
    public async Task ShouldUpsertEnergyCostForMultipleMeasurementsInSameHour()
    {
        ...
        // Second measurement: delta = 1000 Wh = 1 kWh
        ...
        // Third measurement in the same hour: delta = 500 Wh = 0.5 kWh (from m2)
        ...
        // Should still be 1 row (upserted) - the last measurement's delta
        table.Rows.Length.Should().Be(1);
        // The last upsert overwrites with 0.5 kWh (delta from m2 to m3)
        Convert.ToDouble(table.Rows[0].Cells[1].Value).Should().Be(0.5);
    }

    [Fact]
    public async Task ShouldDistributeEnergyAcrossMultipleHours()
    {
        var testLocation = await Factory.CreateTestLocation();
        var client = testLocation.HttpClient;
        var token = testLocation.Token;
        ...
        var hour11 = new DateTime(2024, 1, 15, 11, 0, 0, DateTimeKind.Utc);
        ...
        await InsertEnergyPrice(client, token, testLocation.PriceAreaId, hour11, 2.00m, 1.60m);
        ...
        // Next reading at 12:15 - spans hours 10 and 11 (previousHourStart to currentHourStart exclusive)
        // Delta = 6000 Wh = 6 kWh, distributed evenly across 2 hours = 3 kWh per hour
        ...
        var table = await client.ExecuteDatabaseQuery("SELECT PowerImport, CostInLocalCurrency, CostInLocalCurrencyAfterSubsidy, Hour FROM EnergyCosts ORDER BY Hour", token);
        table.Rows.Length.Should().Be(2);

        // Hour 10: 3 kWh * 1.00 = 3.00
        Convert.ToDouble(table.Rows[0].Cells[0].Value).Should().Be(3.0);
        Convert.ToDecimal(table.Rows[0].Cells[1].Value).Should().Be(3.00m);

        // Hour 11: 3 kWh * 2.00 = 6.00
        Convert.ToDouble(table.Rows[1].Cells[0].Value).Should().Be(3.0);
        Convert.ToDecimal(table.Rows[1].Cells[1].Value).Should().Be(6.00m);
    }
