...
/// </summary>
public class LocationLightingState(long locationId, string locationName, double latitude, double longitude)
{
...
public class OutdoorLightsController(
    IConfiguration configuration,
    IMessageBus messageBus,
    TimeProvider timeProvider,
    ISunCalculationService sunCalculationService,
    IQueryExecutor queryExecutor,
    ILogger<OutdoorLightsController> logger) : IOutdoorLightsController
{
    private readonly OutdoorLightsOptions _options = configuration.GetOutdoorLightsOptions();
    ...
    public async Task CheckAndPublishLightStates()
    {
        ...
        foreach (var locationState in _locationStates.Values)
        {
            ...
            else
            {
                ...
                logger.LogDebug("Outdoor light state unchanged for location '{LocationName}': {State}",
    ...
    private LightState GetLightStateForLocation(LocationLightingState locationState, DateTime now)
    {
        ...
        {
            logger.LogError("Sun times not properly initialized for location '{LocationName}' (ID: {LocationId}). Sunrise: {Sunrise}, Sunset: {Sunset}",
        ...
        {
            logger.LogError(ex, "DateTime arithmetic overflow for location '{LocationName}' (ID: {LocationId}). Sunrise: {Sunrise}, Sunset: {Sunset}, SunriseOffset: {SunriseOffset}, SunsetOffset: {SunsetOffset}",
    ...
    private async Task EnsureLocationsInitializedAsync()
    {
        ...
        try
        {
            var locationCoordinates = await queryExecutor.ExecuteAsync(new GetLocationCoordinatesQuery());
    ...
    private async Task EnsureTodaysSunTimesCalculatedAsync(LocationLightingState locationState, DateTime now)
    {
        ...
        {
            var (sunrise, sunset) = await sunCalculationService.GetSunriseSunsetAsync(
                today,
            ...
            logger.LogDebug("Calculated sun times for location '{LocationName}' on {Date}: Sunrise {Sunrise} UTC, Sunset {Sunset} UTC",
            ...
            {
                logger.LogDebug("Applied offsets for location '{LocationName}': Sunrise offset {SunriseOffset}, Sunset offset {SunsetOffset}",
